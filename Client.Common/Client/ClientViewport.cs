...
public class ClientViewport
{
    ...
    /// </summary>
    public Dictionary<uint, ClientCreature> Creatures = new Dictionary<uint, ClientCreature>();

    /// <summary>
    /// Our VIP list, we must store full creature as the ID is used for knowing if we should display extra info
    /// </summary>
    public Dictionary<uint, ClientCreature> VIPList = new Dictionary<uint, ClientCreature>();
    ...
    /// All open channels
    /// </summary>
    public Dictionary<ushort, ClientChannel> Channels = new Dictionary<ushort, ClientChannel>();
    ...
    /// <returns></returns>
    private ClientCreature GetCreature(uint CreatureID)
    {
    ...
    private void OnVIPState(Packet props)
    {
        ...
        if (!VIPList.TryGetValue((uint)props["CreatureID"], out Creature))
        {
            Creature = new ClientCreature((uint)props["CreatureID"]);
            VIPList[Creature.ID] = Creature;
        }
        Creature.Name = (string)props["Name"];
        Creature.Online = (bool)props["Online"];
    ...
    {
        ClientCreature Creature = VIPList[(uint)props["CreatureID"]];
        if (Creature != null)
        {
    ...
    private void OnCreatureSpeak(Packet props)
    {
        ...
        if (props["ChannelID"] != null && Channels.ContainsKey((ushort)props["ChannelID"]))
        {
            Channel = Channels[(ushort)props["ChannelID"]];
        }
        Channel.Add(Message);
    }
    ...
    {
        ushort ChannelID = (ushort)props["ChannelID"];
